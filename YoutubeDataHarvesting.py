...
if selected == "Home":
    ...
    with col1:
        ...
        st.write(
            '(:green[Note:- This zone **collects data** by using channel id and **store it in the :blue[MongoDB] database]**.)')
        channel_id = st.text_input('**channel id please**')
        st.write('''Get data and store it in the MongoDB database to click below **:red['Get data and store']**.''')
        ...
        if Get_data or st.session_state.Get_state:
            ...
            api_version = 'v3'
            api_key = 'AIzaSyAzuVSySEYK58HB6oaidJZBG0iRtpCUexw'
